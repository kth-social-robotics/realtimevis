function mocapwrite_trc(filename, P, L, tstep, isNans2Zeros, stime, etime, appstart, clap_start, clap_end)
...
% isNans2Zeros to 1 if that behaviour is required
  ...
  fprintf(fid,'DataRate,CameraRate,NumFrames,NumMarkers,Units,OrigDataRate,OrigDataStartFrame,OrigNumFrames,StartTime,EndTime,RecStart,AppStart,ClapStart,ClapEnd\r\n');
  fps = round(1/tstep);
  nframes = size(P,1);
% nmarkers = size(P,3);
  nmarkers = 0;
  ...
  timestart = stime;
  timeend = etime;
  fprintf(fid,'%u,%u,%u,%u,m,%u,%u,%u,%f,%f,%f,%f,%f,%f\r\n', fps, fps, nframes, size(P,3), fps, 1, nframes, timestart, timeend, appstart - 10, appstart, clap_start, clap_end);
